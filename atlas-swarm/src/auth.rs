...
impl UserSigner {
    pub async fn discover() -> io::Result<Self> {
        match SshAgentSigner::discover().await {
            ...
            Err(agent_error) if matches!(agent_error.kind(), io::ErrorKind::NotFound | io::ErrorKind::ConnectionRefused) => Self::from_default_file(),
            Err(error) => Err(error),
        }
    }
    ...
    fn from_default_file() -> io::Result<Self> {
        ...
        let key = ssh_key::PrivateKey::from_openssh(source).map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error))?;
        let private = key.key_data().ed25519().ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "SSH fallback key is not Ed25519"))?;
        Ok(Self::File(ed25519_dalek::SigningKey::from_bytes(&private.private.to_bytes())))
    }

    pub fn user(&self) -> UserId {
        match self { Self::Agent(signer) => signer.user(), Self::File(key) => UserId::from_signing_key(key) }
    }

    pub async fn sign(&self, payload: &[u8]) -> io::Result<UserSignature> {
        match self {
            Self::Agent(signer) => signer.sign(payload).await,
            Self::File(key) => Ok(UserSignature::Ed25519(ed25519_dalek::Signer::sign(key, payload).to_bytes().to_vec())),
        }
    }
}
...
impl SshAgentSigner {
    ...
    pub async fn discover() -> io::Result<Self> {
        ...
        if response.first() != Some(&12) {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "SSH agent rejected identity request"));
        }
        ...
        for _ in 0..count {
            ...
            let Ok(public_key) = <[u8; 32]>::try_from(public_key) else { continue };
            let security_key = algorithm == b"sk-ssh-ed25519@openssh.com";
            if algorithm != b"ssh-ed25519" && !security_key { continue; }
            if security_key && take_string(&mut key)? != SECURITY_KEY_APPLICATION.as_bytes() { continue; }
            if !key.is_empty() { continue; }
            return Ok(Self { socket, key_blob, user: UserId(public_key), security_key });
        }
        Err(io::Error::new(io::ErrorKind::NotFound, "SSH agent has no ssh-ed25519 identity"))
    }
    ...
    pub async fn sign(&self, payload: &[u8]) -> io::Result<UserSignature> {
        ...
        if response.first() != Some(&14) {
            return Err(io::Error::new(io::ErrorKind::PermissionDenied, "SSH agent refused signature"));
        }
        let mut signature_blob = take_string(&mut &response[1..])?;
        let algorithm = take_string(&mut signature_blob)?;
        if (!self.security_key && algorithm != b"ssh-ed25519") || (self.security_key && algorithm != b"sk-ssh-ed25519@openssh.com") {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "SSH agent returned a non-Ed25519 signature"));
        }
        let signature = take_string(&mut signature_blob)?;
        if signature.len() != 64 {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "SSH agent returned an invalid Ed25519 signature"));
        }
        if !self.security_key {
            if !signature_blob.is_empty() { return Err(io::Error::new(io::ErrorKind::InvalidData, "unexpected SSH signature fields")); }
            return Ok(UserSignature::Ed25519(signature.to_vec()));
        }
        let flags = *signature_blob.first().ok_or_else(|| io::Error::new(io::ErrorKind::UnexpectedEof, "truncated security-key signature"))?;
        signature_blob = &signature_blob[1..];
        let counter = take_u32(&mut signature_blob)?;
        if !signature_blob.is_empty() { return Err(io::Error::new(io::ErrorKind::InvalidData, "unexpected security-key signature fields")); }
        Ok(UserSignature::SecurityKeyEd25519 { flags, counter, signature: signature.to_vec() })
    }
}
...
fn take_u32(input: &mut &[u8]) -> io::Result<u32> {
    let bytes: [u8; 4] = input.get(..4).ok_or_else(|| io::Error::new(io::ErrorKind::UnexpectedEof, "truncated SSH agent message"))?.try_into().unwrap();
    *input = &input[4..];
...
fn take_string<'a>(input: &mut &'a [u8]) -> io::Result<&'a [u8]> {
    ...
    let value = input.get(..length).ok_or_else(|| io::Error::new(io::ErrorKind::UnexpectedEof, "truncated SSH agent string"))?;
    *input = &input[length..];
    Ok(value)
}
