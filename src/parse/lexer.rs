...
impl<'input> Lexer<'input> {
    pub fn new(input: &'input str) -> Self {
        Lexer { chars: StringSlicer::new(input) }
    }
    ...
    fn raw_string_literal(&mut self) -> LexerItem<'input> {
        if let Some((start, 'r', _)) = self.chars.next() {
            if let Some((_, '"', content_start)) = self.chars.next() {
                ...
                if end == content_end { // if we never hit a " the end and content_end will be the same
                ...
                Err(Internal(Span::new(self.chars.pos(), self.chars.pos()),
            ...
            Some((_, '.', _)) => { // we have a float!
                ...
                match self.chars.peek() {
                    Some((ch_start,ch,ch_end)) if is_ident_start(ch) => 
                        return Err(BadNumericLiteral(Span::new(ch_start, ch_end),
                ...
                match self.chars.slice(start, float_end).parse::<f64>() {
                    ...
                    Err(_) => Err(BadNumericLiteral(Span::new(start, float_end), 
                                                    "Failed to parse float literal"))
                }
            },
            Some((b_start, 'b', b_end)) => { // we (might!) have a binary integer
                if hdr_end - hdr_start != ByteOffset(2) || first_ch != '0' {
                    return Err(BadNumericLiteral(Span::new(b_start, b_end),
                    ...
                    match i64::from_str_radix(self.chars.slice(binary_start, binary_end), 2) {
                        Ok(val) => Ok((start, 
                                       ...
                                       binary_end)),
                        Err(_) => Err(BadNumericLiteral(Span::new(start, binary_end), 
                                                        "Failed to parse binary literal"))
                    }
                }
            },
            Some((x_start, 'x', x_end)) => { // we (might!) have a hexidecimal integer
                if hdr_end - hdr_start != ByteOffset(2) || first_ch != '0' {
                    return Err(BadNumericLiteral(Span::new(x_start, x_end),
                    ...
                    match i64::from_str_radix(self.chars.slice(hex_start, hex_end), 16) {
                        Ok(val) => Ok((start, 
                                       Token::IntLiteral(if neg { -val } else { val }),
                                       hex_end)),
                        Err(_) => Err(BadNumericLiteral(Span::new(start, hex_end), 
            ...
            _ => {
                ...
                match i64::from_str_radix(self.chars.slice(hdr_start, hdr_end), 10) {
                    Ok(val) => Ok((start, 
                                   Token::IntLiteral(if neg { -val } else { val }),
                                   hdr_end)),
                    Err(_) => Err(BadNumericLiteral(Span::new(start, hdr_end), 
                                                    "Failed to parse hex literal"))
                }
            }
        }
    ...
    fn block_comment(&mut self) -> Option<LexerItem<'input>> {
        ...
        // it is a doc comment
        let doc = if let Some((_,'*',_)) = self.chars.peek() { true } else { false };

        loop {
            let (cend, _) = self.chars.take_while(|ch| ch != '*');
            match self.chars.next() {
                Some((_, '/', end)) => { // we hit the end
    ...
    fn next(&mut self) -> Option<Self::Item> {
        while let Some((start, ch, end)) = self.chars.peek() {
            return Some(match ch {
                ...
                ch if ch.is_whitespace() => { self.chars.next(); continue },
                _ => {
                    Err(UnexpectedChar(Span::new(start, end)))
                }
            })
        }
