use crate::core::lang::{
    ...
    ArgType, ParamType, Symbol
};
use super::value::{Op, RegAddr, CodeBuilder, SegmentBuilder};
use std::collections::{HashSet, HashMap};
use std::iter::FromIterator;

pub trait Compile {
    fn compile<'p>(&self, dest: RegAddr, 
        seg: &mut SegmentBuilder, code: &mut CodeBuilder, env: &SymbolEnv);
}
...
pub trait CompileCond {
    fn compile<'p>(&self, dest: RegAddr, 
        seg: &mut SegmentBuilder, code: &mut CodeBuilder, env: &'p SymbolEnv) -> SymbolEnv<'p>;
}

pub trait CompileEnv {
    fn compile<'p>(&self, seg: &mut SegmentBuilder, code: &mut CodeBuilder,
            env: &'p SymbolEnv) -> SymbolEnv<'p>;
}

impl Compile for Literal {
    fn compile(&self, dest: RegAddr, seg: &mut SegmentBuilder, _: &mut CodeBuilder,
                _: &SymbolEnv<'_>) {
    ...
    fn compile<'p>(&self, dest: RegAddr, seg: &mut SegmentBuilder, code: &mut CodeBuilder,
        ...
        match self {
            Atom::Id(id) => match env.get(id) {
                Some(ptr) => seg.append(Op::Cp(dest, ptr)),
...
impl Compile for Body {
    fn compile<'heap>(&self, dest: RegAddr, seg: &mut SegmentBuilder, code: &mut CodeBuilder,
                            env: &SymbolEnv<'_>) {
        match self {
        ...
        match self {
            ...
            App(l, r) => {
                ...
                for (a, x) in r {
                    ...
                    ArgType::ExpandPos => {
                        seg.append(Op::Force(arg_reg));
                        seg.append(Op::PosVarArg(dest, arg_reg))
                    },
                    ArgType::ExpandKeys => {
                        seg.append(Op::Force(arg_reg));
                        seg.append(Op::KeyVarArg(dest, arg_reg))
                    }
                    }
                }
            },
            ...
            Lam(args , body) => {
                ...
                let mut sub_env = SymbolEnv::child(env);
    ...
    fn compile<'p>(&self, seg: &mut SegmentBuilder, code: &mut CodeBuilder,
        ...
        match self {
            ...
            Bind::Rec(bindings) => {
                ...
                let mut rec_env = SymbolEnv::child(env);
                for (s, _) in bindings {
                ...
                for (res_reg, ep_reg) in 
                        res_registers.iter().zip(ep_registers.iter()) {
                    match res_reg {
...
pub struct SymbolEnv<'p> {
    ...
    pub regs: HashMap<Symbol, RegAddr>
}

impl<'p> SymbolEnv<'p> {
    pub fn new() -> Self {
        SymbolEnv { parent: None, regs: HashMap::new() }
    }

    pub fn child(parent: &'p SymbolEnv<'p>) -> Self {
        SymbolEnv { parent: Some(parent), regs: HashMap::new() }
    }
    ...
    pub fn extend(&mut self, child: HashMap<Symbol, RegAddr>) {
        self.regs.extend(child)
    }

    pub fn set(&mut self, id: Symbol, n: RegAddr) {
        self.regs.insert(id, n);
    }
